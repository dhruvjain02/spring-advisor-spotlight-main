...
const AdvisorCard: React.FC<AdvisorCardProps> = ({ advisor }) => {
  const { id, name, title, bio, imageUrl, location, specializations } = advisor;
  ...
  return (
    <Card className="h-full flex flex-col overflow-hidden hover:shadow-lg transition-shadow duration-300 animate-fade-in">
      ...
      <CardContent className="p-4 flex-grow">
        ...
        <h3 className="font-semibold text-xl mb-1 text-[#272A2B]">{name}</h3>
        <p className="text-sm text-gray-500 mb-3">{title}</p>
        
        <p className="text-sm mb-4 line-clamp-2 text-[#272A2B]">{bio}</p>
